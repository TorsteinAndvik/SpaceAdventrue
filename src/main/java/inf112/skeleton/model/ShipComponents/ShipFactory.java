...
public class ShipFactory {
  ...
  public ShipConfig createShipConfigFromJson(JsonValue shipData) {
    ...
    for (JsonValue component : shipData.get("components")) {
      ...
      upgrade.type = UpgradeType.getUpgradeType(component.get("upgrade").get("type").asString());
    ...
    if (!ShipValidator.validShipConfig(shipConfig)) {
      throw new IllegalArgumentException("Invalid ship configuration for " + shipData.get("name"));
    }
