...
public class UpgradeScreen extends InputAdapter implements Screen {
  ...
  private final AssetManager manager;   // An assetmanager helps with loading assets and disposing them once they are no longer needed
  private final UpgradeScreenModel model;
  private final UpgradeScreenController controller;
  private final Vector2 touchPos;       // Simplifies converting touch / mouse position in window-coordinates (pixels) to game-coordinates (meters x meters set in viewport)
  ...
  public void render(float delta) {
    ...
    if (model.isCameraZoomRecently()) {
      float alpha = model.getCameraZoomDeltaTime() < model.getCameraZoomTextFadeCutoffTime() ?
          1f : 1f - (float) Math.pow(
  ...
  /**
   * Convert screen coordinate to world coordinates. Windows have origin at top-left, game world at
   * bottom-left.
   *
